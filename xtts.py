...
# from TTS.tts.layers.xtts.gpt6 import GPT
# from TTS.tts.layers.xtts.gpt5 import GPT
# from TTS.tts.layers.xtts.gpt4 import GPT
# from TTS.tts.layers.xtts.gpt3 import GPT
from TTS.tts.layers.xtts.gpt2 import GPT
...
class Xtts(BaseTTS):
    ...
    def get_gpt_cond_latents(self, audio, sr, length: int = 30, chunk_length: int = 6, motion_feat=None):
        ...
        Args:
            ...
            motion_feat : slow_fast features path
        """
        if sr != 22050:
        ...
        if self.args.gpt_use_perceiver_resampler:
            ...
            for i in range(0, audio.shape[1], 22050 * chunk_length):
                ...
                if motion_feat is not None:
                    motion_feat = torch.tensor(np.load(motion_feat)).to(self.device).unsqueeze(0)
                # pass the videoCLIP features also to the style encoder
                # style_emb1, style_emb2, style_emb3 = self.gpt.get_style_emb(mel_chunk.to(self.device), None, motion_feat=motion_feat)
                style_emb = self.gpt.get_style_emb(mel_chunk.to(self.device), None, motion_feat=motion_feat)
        ...
        # return style_emb1, style_emb2, style_emb3
    ...
    def get_conditioning_latents(
        ...
        if audio_path_spkr_ref is not None:
            audio = load_audio(audio_path_spkr_ref, load_sr)
        ...
        for file_path in audio_paths:
            ...
            # compute latents for the decoder
            if audio_path_spkr_ref is None:
                speaker_embedding = self.get_speaker_embedding(audio, load_sr)
                speaker_embeddings.append(speaker_embedding)
        ...
        # gpt_cond_latents1, gpt_cond_latents2, gpt_cond_latents3 = self.get_gpt_cond_latents(            
        #     full_audio, load_sr, length=gpt_cond_len, chunk_length=gpt_cond_chunk_len, motion_feat=motion_feat
        # )  # [1, 1024, T]
        ...
        # return gpt_cond_latents1, gpt_cond_latents2, gpt_cond_latents3
    
    def synthesize(self, text, config, speaker_wav, language, motion_feat=None,posefeat=None, **kwargs):
        ...
        """
        return self.inference_with_config(text, config, ref_audio_path=speaker_wav, language=language, motion_feat=motion_feat, posefeat=posefeat,**kwargs)

    def inference_with_config(self, text, config, ref_audio_path, language, motion_feat=None,posefeat=None, **kwargs):
        """
        ...
        return self.full_inference(text, ref_audio_path, language, motion_feat=motion_feat,posefeat=posefeat, **settings)
    ...
    def full_inference(
        ...
        **hf_generate_kwargs,
        ...
        Args:
            ...
            motion_feat: SlowFast features
            posefeat: Openpose features
    ...
    def inference(
        ...
        **hf_generate_kwargs,
        ...
        for sent in text:
            ...
            # pass the AV-HuBERT video features as the encoder_hidden_states to do cross-attention over
            encoder_hidden_states = torch.tensor(np.load(posefeat)).unsqueeze(0).to(text_tokens.device)
            with torch.no_grad():
